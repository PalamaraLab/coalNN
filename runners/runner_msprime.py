...
class MsprimeRunner(Runner):
    def create_data(self):
        ...
        num_pairs = int((num_haplotypes * (num_haplotypes - 1)) / 2)
        all_pairs = [pair
                     for pair in self.run_dataset.simulations[0]['haplotypes_to_pair']]

        if hasattr(self.config, 'saliency_map') and self.config.saliency_map:
            if hasattr(self.config, 'n_pairs_saliency_map'):
                self.random_pairs = random.sample(all_pairs, k=self.config.n_pairs_saliency_map)
            else:
                self.random_pairs = all_pairs[0]
        else:
